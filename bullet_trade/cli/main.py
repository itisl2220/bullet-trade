...
示例:
  ...
  更多信息请访问: https://github.com/BulletTrade/bullet-trade
    ...
    # gui 命令
    gui_parser = subparsers.add_parser("gui", help="启动图形化用户界面")

    # strategies 命令：列出远程策略 / 选择用于回测的 remote://<id>
    strat_parser = subparsers.add_parser("strategies", help="远程策略管理：列出/选择远程托管的策略")
    strat_parser.add_argument(
        "--server", dest="server", default=None, help="后端地址，例如 http://host:port"
    )
    strat_parser.add_argument(
        "--token", dest="token", default=None, help="访问 token（覆盖环境变量）"
    )
    strat_parser.add_argument("--list", dest="list", action="store_true", help="列出可用策略")
    strat_parser.add_argument(
        "--use", dest="use", default=None, help="选择策略 ID 并输出 remote://<id>"
    )
    strat_parser.add_argument(
        "--save",
        dest="save",
        action="store_true",
        help="将 remote://<id> 保存到 .remote_strategy 文件",
    )

    return parser


def _get_strategy_server_base_from_args_or_env(args) -> str:
    return (
        getattr(args, "server", None)
        or os.getenv("STRATEGY_SERVER_URL")
        or os.getenv("BT_SERVER_URL")
        or "http://127.0.0.1:3000"
    )


def _fetch_json_from_url(url: str, token: Optional[str] = None):
    import requests
...
def run_strategies_command(args):
    """处理 strategies 子命令：列出 / 选择远程策略"""
    server = _get_strategy_server_base_from_args_or_env(args).rstrip("/")
    token = (
        getattr(args, "token", None) or os.getenv("STRATEGY_API_TOKEN") or os.getenv("BT_API_TOKEN")
    )
    if getattr(args, "list", False):
        url = f"{server}/api/strategies"
        try:
            data = _fetch_json_from_url(url, token)
    ...
    if getattr(args, "use", None):
        ...
        if getattr(args, "save", False):
            try:
                with open(".remote_strategy", "w", encoding="utf-8") as f:
                    f.write(remote)
                print("已写入 .remote_strategy")
            except Exception as e:
                print(f"写入文件失败: {e}")
                return 1
        return 0
    print("请使用 --list 列出策略或 --use <id> 选择策略")
    return 1
...
def main():
    ...
    if args.command == "backtest":
        ...
        return run_strategies_command(args)
