...
def extract_strategy_params_from_source(source: str) -> Dict[str, Dict[str, Any]]:
    """
    从源码字符串提取策略参数（与 extract_strategy_params 类似，但不读取文件）。
    """
    if not source:
        return {}
    ...
    try:
        ...
        params.update(_extract_from_ast(tree, source))
    except Exception:
        pass
    try:
        params.update(_extract_from_regex(source))
    except Exception:
        pass
    return params
