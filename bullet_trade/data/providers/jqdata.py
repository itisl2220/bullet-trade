...
# 在导入 jqdatasdk 之前清理环境变量中的注释
# 防止 jqdatasdk 库在导入时读取到包含注释的环境变量
def _clean_env_for_jqdatasdk():
    """清理环境变量中的注释，防止 jqdatasdk 库内部解析出错"""
    # 清理所有可能包含注释的 JQData 相关环境变量
    env_keys_to_clean = ["JQDATA_PORT", "JQDATA_SERVER"]

    for key in env_keys_to_clean:
        value = os.environ.get(key)
        if value and isinstance(value, str):
            # 如果整个值都是注释（以 # 开头），删除环境变量
...
# 在导入 jqdatasdk 之前清理环境变量
_clean_env_for_jqdatasdk()

# 现在可以安全地导入 jqdatasdk（必须在清理环境变量之后）
# 注意：这些导入必须在清理环境变量之后，因此不能放在文件顶部
import jqdatasdk as jq  # noqa: E402
from jqdatasdk import finance, query  # noqa: E402
...
class JQDataProvider(DataProvider):
    ...
    def auth(
        ...
        cfg_server = self.config.get("server") or get_env("JQDATA_SERVER", "")
        jq_server = self._parse_host(host, cfg_server)
        cfg_port = self.config.get("port")
        jq_port_env = str(cfg_port) if cfg_port is not None else get_env("JQDATA_PORT", "")
        jq_port = self._parse_port(port, jq_port_env)
        jq_user = (
            user
            or self.config.get("username")
            or get_env("JQDATA_USERNAME")
            or get_env("JQDATA_USER", "")
        )
        jq_pwd = (
            pwd
            or self.config.get("password")
            or get_env("JQDATA_PASSWORD")
            or get_env("JQDATA_PWD", "")
        )
        # 允许空host/port走默认
        if jq_user:
            try:
    ...
    def get_price(
        ...
        if should_try_engine and self._price_engine_supported is not False:
            try:
                result = self._cache.cached_call(
                    "get_price_engine", kwargs, _fetch_price_engine, result_type="df"
                )
    ...
    def get_trade_days(
        ...
        def _fetch(kw: Dict[str, Any]) -> List[datetime]:
            return jq.get_trade_days(
                start_date=kw.get("start_date"), end_date=kw.get("end_date"), count=kw.get("count")
            )
        ...
        try:
                ...
                if hasattr(jq, "get_current_tick"):
                    ...
                    if tick is not None:
                        last_price = (
                            ...
                            else getattr(tick, "last_price", None)
                        )
            except Exception:
                pass
            ...
            if (not high_limit) or (not low_limit):
                try:
                    # 昨收
                    daily = jq.get_price(
                        security, count=2, end_date=today, frequency="daily", fields=["close"]
                    )
                    pre_close = None
                    ...
                    try:
                        if hasattr(jq, "get_extras"):
                            st_df = jq.get_extras(
                                "is_st", [security], start_date=today, end_date=today
                            )
            ...
            try:
                if hasattr(jq, "get_extras"):
                    paused_df = None
                    try:
                        paused_df = jq.get_extras(
                            "is_paused", [security], start_date=today, end_date=today
                        )
                    except Exception:
                        paused_df = None
                    if paused_df is None or paused_df.empty:
                        try:
                            paused_df = jq.get_extras(
                                "paused", [security], start_date=today, end_date=today
                            )
                        except Exception:
                            paused_df = None
            ...
            try:
                if sec_type in ("fja", "fjb"):
                    ...
                    q = query(table).filter(
                        table.code == code_num, table.day >= sd, table.day <= ed
                    )
                    ...
                    try:
                        ...
                        table = _FINANCE_TABLE_STUB
                    q = query(table).filter(
                        table.code == code_num, table.ex_date >= sd, table.ex_date <= ed
                    )
                    df = finance.run_query(q)
                    for _, row in df.iterrows():
                    ...
                    try:
                        ...
                        table = _FINANCE_TABLE_STUB
                    q = query(table).filter(
                        table.code == security_i, table.a_xr_date >= sd, table.a_xr_date <= ed
                    )
                    df = finance.run_query(q)
                    for _, row in df.iterrows():
    ...
    def _adjust_long_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        working = df.copy()
        if (
            "time" not in working.columns
            or "code" not in working.columns
            or "factor" not in working.columns
        ):
            return working
    ...
    def _compute_ratio_series(self, series: pd.Series) -> pd.Series:
        if series is None or series.empty:
            return pd.Series(
                [], index=series.index if isinstance(series, pd.Series) else None, dtype=float
            )
